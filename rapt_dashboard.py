...
function showYeastInfo(name) {
    ...
    fetch('/yeast_list')
        ...
        .then(data => {
            ...
            const y = (data.yeasts || []).find(y => y.name === name)
        ...
        .catch(() => {
            loading.classList.add('hidden')
            document.getElementById('yim_name').textContent = 'Failed to load yeast data'
        })
}
        ...
        .then(result => {
            ...
            const infoBtn = yeast !== '--'
                ? ` &nbsp;<button onclick="showYeastInfo('${yeast.replace(/'/g, "\\'")}')" title="Yeast info"
                    class="ml-2 px-2 py-0.5 rounded-lg bg-zinc-700 hover:bg-zinc-600 text-zinc-300 text-sm font-semibold align-middle">ℹ️</button>`
                : ''
            yeastEl.innerHTML = `🧫 Yeast: <span class="font-bold text-zinc-200">${yeast}</span> &nbsp;|&nbsp; 🌡️ Fermentation Temp: <span class="font-bold text-zinc-200">${temp}</span>${infoBtn}`

            const diacetylEl = document.getElementById('diacetylStatus')
            const diacetylTemp = result.fermentation_temp != null ? `${(result.fermentation_temp + 3).toFixed(1)}°C` : '--'
