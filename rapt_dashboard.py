...
def ensure_log_file():
    ...
    # If file doesn't exist, create an empty valid JSON list
...
def append_log_entry(timestamp, raw_sg, temp_c):
    ...
    if os.path.exists(filename):
        try:
            ...
            # Backup corrupted file instead of wiping it
...
def start_brew():
    ...
    # Guarantee valid timestamp
    if start_date and start_time:
        session_start = f"{start_date}T{start_time}"
    else:
        # fallback: NOW in UK time
