...
ORIGINAL_GRAVITY = 1.0513   # Your starting gravity
...
if data_to_send:
    try:
        # 1) If RAPT already sent ABV, use that (and just format it)
        if 'abv' in data_to_send and data_to_send['abv'] is not None:
            data_to_send['abv'] = round(float(data_to_send['abv']), 3)
        else:
            # 2) Fallback: only calculate if ABV not provided
