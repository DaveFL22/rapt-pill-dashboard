...
function updateConditioningDisplay(result) {
    ...
    function tickConditioning() {
        ...
        const remainingHrsOnly = remainingHours % 24
        const elapsedHours = Math.floor(elapsedMs / (1000 * 60 * 60))
        const elapsedDays = Math.floor(elapsedHours / 24)
        const elapsedHrsOnly = elapsedHours % 24
        ...
        statusEl.innerHTML = `<div class="flex justify-between flex-wrap gap-2 w-full">
            ...
            <span>⏱️ Time remaining: <span class="font-bold text-blue-300">${remainingDays}d ${remainingHrsOnly}h</span></span>
...
def set_cold_crash_start():
    try:
        cfg = load_config()
        cfg["cold_crash_start"] = datetime.now(uk).isoformat(timespec="minutes")
...
def set_conditioning_start():
    try:
        cfg = load_config()
        cfg["cold_conditioning_start"] = datetime.now(uk).isoformat(timespec="minutes")
