...
<body class="bg-zinc-950 text-white min-h-screen p-6">
    ...
    <!-- BREW STARTED -->
    <div id="brewStarted" class="max-w-5xl mx-auto mb-2 p-5 rounded-3xl bg-zinc-900 text-lg font-medium text-zinc-400">
        🍺 Brew started: --
    </div>
    ...
    fetch('/latest')
        ...
        .then(result => {
            ...
            if (result.session_start) {
                ...
                document.getElementById('brewStarted').textContent = `🍺 ${name} brew started on: ${formatted}`
            }
...
def get_latest():
        ...
        if os.path.exists(csv_file):
            with open(csv_file, "r") as f:
                ...
                if rows:
                    ...
                    return jsonify({"data": fallback, "timestamp": last["timestamp"], "session_start": cfg.get("session_start", ""), "profile_name": cfg.get("profile_name", "")})
    ...
    return jsonify({"data": data_to_send, "timestamp": ts, "session_start": cfg.get("session_start", ""), "profile_name": cfg.get("profile_name", "")})
