    ...
    fetch('/latest')
        ...
        .then(result => {
            ...
            if (result.diacetyl_rest) {
                diacetylEl.innerHTML = `🧪 Diacetyl Rest: <span class="font-bold text-green-400">Yes</span> &nbsp;|&nbsp; 🌡️ Rest Temp: <span class="font-bold text-green-400">${diacetylTemp}</span>`
...
def get_latest():
        ...
        if os.path.exists(csv_file):
            with open(csv_file, "r") as f:
                ...
                if rows:
                    ...
                    return jsonify({"data": fallback, "timestamp": last["timestamp"], "session_start": cfg.get("session_start", ""), "profile_name": cfg.get("profile_name", ""), "diacetyl_rest": cfg.get("diacetyl_rest", False), "yeast_name": cfg.get("yeast_name", ""), "fermentation_temp": cfg.get("fermentation_temp")})
    ...
    return jsonify({"data": data_to_send, "timestamp": ts, "session_start": cfg.get("session_start", ""), "profile_name": cfg.get("profile_name", ""), "diacetyl_rest": cfg.get("diacetyl_rest", False), "yeast_name": cfg.get("yeast_name", ""), "fermentation_temp": cfg.get("fermentation_temp")})
