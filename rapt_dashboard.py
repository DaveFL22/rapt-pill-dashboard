...
# ============================================================
# DASHBOARD HTML TEMPLATE (with temperature offset control)
# ============================================================
...
<body class="bg-zinc-950 text-white min-h-screen p-6">
    ...
    <div class="max-w-5xl mx-auto flex flex-col gap-3 md:flex-row md:justify-between md:items-center mb-6">
        ...
        <div class="flex flex-wrap gap-3 justify-end">
            ...
            <button onclick="pushToGitHub()" class="bg-zinc-800 hover:bg-zinc-700 text-emerald-400 text-sm px-4 py-3 rounded-2xl">
                ⬆ Upload Log to GitHub
            </button>
            <button onclick="pullFromGitHub()" class="bg-zinc-800 hover:bg-zinc-700 text-amber-300 text-sm px-4 py-3 rounded-2xl">
                ⬇ Pull Log from GitHub
            </button>
            <button onclick="openModal()" class="bg-amber-400 hover:bg-amber-300 text-black font-semibold px-6 py-3 rounded-2xl">
                + Start New Brew
            </button>
        </div>
    </div>
        ...
        <div class="bg-zinc-900 p-8 rounded-3xl w-full max-w-lg">
            ...
            <form id="brewForm">
                ...
                <label class="block mb-3">
                    ...
                    <input name="original_gravity" type="number" step="0.001" class="w-full mt-1 p-3 rounded-xl bg-zinc-800 text-white" />
                </label>
                <label class="block mb-3">
                    <span class="text-zinc-300">Start Date</span>
                    <input name="start_date" type="date" value="{{ today }}" class="w-full mt-1 p-3 rounded-xl bg-zinc-800 text-white" />
                </label>
                <label class="block mb-3">
                    <span class="text-zinc-300">Start Time</span>
                    <input name="start_time" type="time" value="{{ now }}" class="w-full mt-1 p-3 rounded-xl bg-zinc-800 text-white" />
                </label>
                <label class="block mb-6">
                    <span class="text-zinc-300">Calibration Offset (Gravity)</span>
                    <input name="calibration_offset" type="number" step="0.0001" value="0.0000" class="w-full mt-1 p-3 rounded-xl bg-zinc-800 text-white" />
                </label>
...
function pushToGitHub() {
    ...
    fetch('/push_to_github', { method: 'POST' })
        .then(r => r.json())
        .then(res => res.success ? setGhStatus('✅ Log uploaded to GitHub.') : setGhStatus('❌ Upload failed: ' + (res.error || 'Unknown error'), false));
}
...
def get_latest():
    ...
    try:
        ...
        data_to_send["temperature"] = corrected_temp
...
def append_log_entry(timestamp, raw_sg, raw_temp):
    ...
    with open(filename, "a", newline="") as f:
        ...
        writer.writerow([timestamp.astimezone(uk).isoformat(), raw_sg, raw_temp])
...
def download_csv():
    ...
    if not os.path.exists(filename):
        ...
        with open(filename, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "gravity", "temperature_raw"])
    return send_file(filename, as_attachment=True, download_name=os.path.basename(filename), mimetype="text/csv")
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            data.append({
                ...
                "temperature": float(row.get("temperature_raw", row.get("temperature", 0)))
            })
...
def push_to_github():
    ...
    try:
        ...
        if len(content_bytes) < 10:
            return jsonify({"success": False, "error": "CSV too small"}), 500
        ...
        return jsonify({"success": True}) if resp and resp.status_code in (200, 201) else jsonify({"success": False, "error": "GitHub error"}), 500
    except Exception as e:
...
def pull_from_github():
    ...
    try:
        ...
        if len(content_bytes) < 10:
            return jsonify({"success": False, "error": "GitHub file too small"}), 500
...
# ============================================================
# VIEW LOG ROUTE (correctly placed at the bottom)
# ============================================================
@app.route("/view_log")
def view_log():
    csv_file = get_current_brew_log_csv_filename()
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="range-row">
            <div><label>Start</label><br><input type="datetime-local" id="startRange"></div>
            <div><label>End</label><br><input type="datetime-local" id="endRange"></div>
            <div><label>&nbsp;</label><br><button class="btn btn-primary" onclick="applyCustomRange()">Apply</button></div>
        </div>
    ...
    <div class="card">
        <div class="chart-header"><div class="chart-title">Gravity</div><button class="btn" onclick="resetZoom('gravity')">Reset Zoom</button></div>
        <div class="chart-container"><canvas id="gravityChart"></canvas></div>
    </div>
    <div class="card">
        <div class="chart-header"><div class="chart-title">Temperature</div><button class="btn" onclick="resetZoom('temp')">Reset Zoom</button></div>
...
<script>
    ...
    function filterData(start, end) {{ return fullData.filter(row => new Date(row.timestamp).getTime() >= start && new Date(row.timestamp).getTime() <= end); }}
    function applyPreset(hours) {{ const end = Date.now(); const start = end - (hours * 3600 * 1000); render(filterData(start, end)); }}
    ...
    function resetFilter() {{ render(fullData); }}
    function computeDropPoints(data) {{
        const dropPoints = [];
        const threshold = 0.0005;
        for (let i = 1; i < data.length; i++) {{
            const drop = data[i-1].gravity - data[i].gravity;
            if (drop > threshold) dropPoints.push({{ x: data[i].timestamp, y: data[i].gravity }});
        }}
        return dropPoints;
    }}
    function render(data) {{
        ...
        gravityChart.data.labels = ts; gravityChart.data.datasets[0].data = gs; gravityChart.data.datasets[1].data = dropPoints; gravityChart.update();
    ...
    function pushToGitHubLog() {{ /* same as above */ setGhStatusLog('Uploading...', true); fetch('/push_to_github', {{method:'POST'}}).then(r=>r.json()).then(res=> res.success ? setGhStatusLog('✅ Uploaded') : setGhStatusLog('❌ Failed', false)); }}
    function pullFromGitHubLog() {{ setGhStatusLog('Pulling...', true); fetch('/pull_from_github', {{method:'POST'}}).then(r=>r.json()).then(res=>{{ if(res.success){{setGhStatusLog('✅ Pulled'); window.location.reload();}} else setGhStatusLog('❌ Failed', false); }}); }}
    const gravityChart = new Chart(document.getElementById('gravityChart').getContext('2d'), {{ type: 'line', data: {{ labels: {timestamps}, datasets: [{{label:'Gravity',data:{gravities},borderColor:'#22c55e',tension:0.25}},{{label:'Rapid Drop',data:[],pointRadius:6,pointBackgroundColor:'red',showLine:false,type:'scatter'}}] }}, options: {{responsive:true,maintainAspectRatio:false,scales:{{x:{{ticks:{{color:'#9ca3af'}},grid:{{color:'#111827'}}}},y:{{ticks:{{color:'#9ca3af'}},grid:{{color:'#111827'}}}}}},plugins:{{legend:{{labels:{{color:'#e5e7eb'}}}},zoom:{{zoom:{{wheel:{{enabled:true}},pinch:{{enabled:true}},mode:'x'}},pan:{{enabled:true,mode:'x'}}}}}} }});
    const tempChart = new Chart(document.getElementById('tempChart').getContext('2d'), {{ type: 'line', data: {{ labels: {timestamps}, datasets: [{{label:'Temperature (°C)',data:{temps},borderColor:'#fbbf24',tension:0.25}}] }}, options: {{responsive:true,maintainAspectRatio:false,scales:{{x:{{ticks:{{color:'#9ca3af'}},grid:{{color:'#111827'}}}},y:{{ticks:{{color:'#9ca3af'}},grid:{{color:'#111827'}}}}}},plugins:{{legend:{{labels:{{color:'#e5e7eb'}}}},zoom:{{zoom:{{wheel:{{enabled:true}},pinch:{{enabled:true}},mode:'x'}},pan:{{enabled:true,mode:'x'}}}}}} }});
    render(fullData);
</script>
</body>
</html>
