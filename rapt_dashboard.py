...
# ============================================================
#  LOAD CONFIG.JSON (profile name, OG, session start, offset)
# ============================================================
...
PROFILE_NAME = config.get("profile_name", "Unknown Beer")
ORIGINAL_GRAVITY = float(config.get("original_gravity", 1.050))
MANUAL_SESSION_START = config.get("session_start", "2026-01-01T00:00:00")
CALIBRATION_OFFSET = float(config.get("calibration_offset", 0.0000))
# ============================================================
...
<body class="bg-zinc-950 text-white min-h-screen p-6">
    <div class="max-w-5xl mx-auto">
        <h1 class="text-4xl font-semibold mb-2">{{ profile_name }} - RAPT Pill Dashboard</h1>
        <p class="text-zinc-400 mb-6">Live Fermentation Monitor • OG: {{ original_gravity }}</p>
...
def get_latest():
    ...
    if data_to_send:
        try:
            ...
            # --- ALWAYS USE CONFIG.JSON SESSION START ---
            try:
                ...
                print("Session start parse error:", e)
...
def webhook():
    ...
    try:
        ...
        # Remove session_start if present (we do NOT trust it)
