# ============================================================
#  BLOCK F1 — IMPORTS, CONFIG, HELPERS
# ============================================================
...
# ============================================================
#  BLOCK F2 — FULL HTML TEMPLATE (WITH TEMP OFFSET CARD)
# ============================================================
...
<body class="bg-zinc-950 text-white min-h-screen p-6">
    ...
    <div class="max-w-5xl mx-auto mt-6">
        <div class="bg-zinc-900 rounded-3xl p-6 card">
            <p class="text-zinc-400 text-sm mb-1">Temp Offset</p>

            <p id="tempOffsetDisplay" class="text-2xl font-semibold mb-4">
                +0.00°C
            </p>

            <div class="flex gap-3">
                <button onclick="adjustTempOffset(-0.1)"
                    class="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-xl">
                    -0.1
                </button>
...
function saveTempOffset() {
    ...
    .then(res => {
        if (res.success) {
            setGhStatus("Temp offset saved.", true);
            refreshData();
        } else {
...
function refreshData() {
    ...
    fetch('/latest')
        ...
        .then(result => {
            ...
            tempOffset = d.temp_offset || 0.0;
            updateTempOffsetDisplay();

            let correctedTemp = "--";
            if (d.temperature !== undefined && d.temperature !== null) {
                correctedTemp = (parseFloat(d.temperature) + tempOffset).toFixed(2);
            }

            document.getElementById('temp').innerHTML =
                `${correctedTemp}<span class="unit">°C</span>`
...
# ============================================================
#  BLOCK F3 — DASHBOARD ROUTES (INCLUDING BULLETPROOF WEBHOOK)
# ============================================================
...
def get_latest():
            ...
            with open(csv_file, "r") as f:
                ...
                if rows:
                    ...
                    fallback = {
                        ...
                        "battery": "--",
                        "session_length": "--",
                        "temp_offset": cfg["calibration_offset"],
                    }
    ...
    try:
        ...
        # Temperature offset returned to frontend
        data_to_send["temp_offset"] = cfg["calibration_offset"]
...
# ============================================================
#  WEBHOOK — BULLETPROOF VERSION (NEVER RETURNS 400)
# ============================================================
...
def webhook():
    ...
    try:
        # Read raw body safely (works with ANY content-type)
        ...
        # Always return 200 so RAPT never shows an error
        return jsonify({"success": True}), 200

    except Exception as e:
        print("Webhook error:", e)
        # Still return 200 so RAPT doesn't fail
...
# ============================================================
#  SET TEMPERATURE OFFSET (NEW, BULLETPROOF)
# ============================================================
...
def set_temp_offset():
    ...
    cfg = get_config()
    cfg["calibration_offset"] = new_offset
...
# ============================================================
#  BLOCK F4 — GITHUB SYNC, KEEPALIVE, CSV RESTORE
# ============================================================
...
# Run restore on startup in a background thread
threading.Thread(
...
# ============================================================
#  BLOCK F5 — LOG VIEWER + FINAL RUNNER
# ============================================================
