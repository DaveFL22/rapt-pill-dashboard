...
# ============================================================
#  KEEPALIVE THREAD (prevents Render sleeping)
# ============================================================
...
def keepalive():
    while True:
        try:
            requests.get(RENDER_HEALTH_URL, timeout=5)
        except Exception as e:
...
# ============================================================
#  GITHUB PUSH / PULL FOR CURRENT BREW CSV
# ============================================================
def _github_headers():
...
def push_to_github():
    ...
    try:
        ...
        sha = None
        get_resp = requests.get(base_url, headers=headers, params={"ref": GITHUB_BRANCH})
        if get_resp.status_code == 200:
            sha = get_resp.json().get("sha")

        payload = {
            "message": f"Upload fermentation log {filename}",
            "content": content_b64,
        ...
        put_resp = requests.put(base_url, headers=headers, json=payload)
        if put_resp.status_code in (200, 201):
            return jsonify({"success": True}), 200
        else:
            return jsonify({
                "success": False,
                "error": f"GitHub API error: {put_resp.status_code} {put_resp.text}"
            }), 500
...
def pull_from_github():
    ...
    try:
        resp = requests.get(base_url, headers=headers, params={"ref": GITHUB_BRANCH})
        if resp.status_code != 200:
            return jsonify({
                "success": False,
                "error": f"GitHub file not found or error: {resp.status_code}"
            }), 404

        body = resp.json()
        content_b64 = body.get("content", "")
        if not content_b64:
            return jsonify({"success": False, "error": "Empty content from GitHub"}), 500
...
# ============================================================
#  AUTO‑PUSH BACKGROUND FUNCTION (used by webhook)
# ============================================================
def push_csv_to_github_background():
    """Background thread wrapper for auto-push."""
    try:
        headers = _github_headers()
        if headers is None:
        ...
        csv_file = get_current_brew_log_csv_filename()
        if not os.path.exists(csv_file):
            print("Auto-push skipped: CSV file missing")
            return
        ...
        if put_resp.status_code in (200, 201):
            ...
            print("Auto-push failed:", put_resp.status_code, put_resp.text)
