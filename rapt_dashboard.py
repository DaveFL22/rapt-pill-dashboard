    ...
    fetch('/latest')
        ...
        .then(result => {
            ...
            if (result.session_start) {
                ...
                document.getElementById('brewStarted').textContent = `🍺 Brew started: ${formatted}`
            }
...
def get_latest():
        ...
        if os.path.exists(csv_file):
            with open(csv_file, "r") as f:
                ...
                if rows:
                    ...
                    return jsonify({"data": fallback, "timestamp": last["timestamp"], "session_start": cfg.get("session_start", "")})
    ...
    return jsonify({"data": data_to_send, "timestamp": ts, "session_start": cfg.get("session_start", "")})
