...
def save_session(start_time):
    with open(SESSION_FILE, "w") as f:
        json.dump({"session_start": start_time.isoformat()}, f)

session_start = load_session()
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            <div class="card bg-zinc-900 rounded-3xl p-8">
                ...
                <p id="temp" class="text-5xl font-semibold mt-4">--<span class="unit">°C</span></p>
            </div>
            <div class="card bg-zinc-900 rounded-3xl p-8">
                <p class="text-zinc-400 text-sm">SPECIFIC GRAVITY</p>
                <p id="gravity" class="text-5xl font-semibold mt-4">1.----</p>
            </div>
            <div class="card bg-zinc-900 rounded-3xl p-8">
                <p class="text-zinc-400 text-sm">ESTIMATED ABV</p>
                <p id="abv" class="text-5xl font-semibold mt-4">--<span class="unit">%</span></p>
            </div>
            <div class="card bg-zinc-900 rounded-3xl p-8">
                <p class="text-zinc-400 text-sm">BATTERY</p>
                <p id="battery" class="text-5xl font-semibold mt-4">--<span class="unit">%</span></p>
            </div>
            <div class="card bg-zinc-900 rounded-3xl p-8">
                <p class="text-zinc-400 text-sm">SESSION LENGTH</p>
                <p id="session" class="text-3xl font-semibold mt-4">--</p>
            </div>
        </div>
            ...
            fetch('/latest')
                ...
                .then(result => {
                    ...
                    document.getElementById('temp').innerHTML =
                        `${d.temperature || d.temp || '--'}<span class="unit">°C</span>`;
                    ...
                    document.getElementById('battery').innerHTML =
                        `${Math.round(d.battery || d.batteryLevel || 0)}<span class="unit">%</span>`;
...
def webhook():
    global latest_data, last_received_time, session_start
    try:
