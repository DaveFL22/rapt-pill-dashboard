    ...
    if (count === 2) {
        ...
        if (stage === '') {
            ...
            addRow(rows1, `<span class="text-yellow-400 font-bold">⚠️ Add first dry hop now</span> — goes in at the start of the Diacetyl Rest
                <button onclick="setDryHopStage('hop1_added')" class="ml-4 px-3 py-1 rounded-xl bg-green-700 hover:bg-green-600 text-white text-sm font-semibold">✅ First Hop Added</button>`, false)
        } else {
            // First hop already added
            addRow(rows1, `✅ First dry hop added${stageTime ? ' at ' + fmt(stageTime) : ''} — <span class="text-green-400">fermenting</span>`, false)
        }
        ...
        if (stage === 'drop_to_14' || stage === 'hopping' || stage === 'complete') {
            ...
            renderPostDiacetylHopRows(rows2, stage, stageTime, now, fmt, count)
        } else {
...
function renderPostDiacetylHopRows(rows, stage, stageTime, now, fmt, count) {
    ...
    if (stage === 'drop_to_14') {
        ...
        if (over) {
            addRow(`<span class="text-yellow-400 font-bold">⚠️ 14°C hold complete!</span> — <span class="font-bold">Add ${count === 2 ? 'second ' : ''}dry hop now</span>
                <button onclick="setDryHopStage('hopping')" class="ml-4 px-3 py-1 rounded-xl bg-green-700 hover:bg-green-600 text-white text-sm font-semibold">✅ Hop${count === 2 ? 's' : ''} Added</button>`)
        } else {
            const h = Math.floor(remaining / (1000 * 60 * 60))
            const m = Math.floor((remaining % (1000 * 60 * 60)) / (1000 * 60))
            addRow(`🌡️ Holding at <span class="font-bold text-blue-300">14°C</span> — add ${count === 2 ? 'second ' : ''}dry hop in <span class="font-bold text-blue-300">${h}h ${m}m</span>`)
        }
        ...
        const remaining = due - now
        const over = remaining <= 0
        addRow(`✅ ${count === 2 ? 'Second' : ''} dry hop added at ${fmt(stageTime)}`)
        if (over) {
...
def yeast_list():
    ...
    try:
        ...
        # Use download_url if content field is missing or truncated
        ...
        if content_b64:
            ...
            if not download_url:
                return jsonify({"error": "No download URL available"}), 500
