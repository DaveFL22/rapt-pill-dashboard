...
document.getElementById('brewForm').onsubmit = function(e) {
    ...
    fetch('/start_brew', { method: 'POST', body: formData }).then(() => { closeModal(); refreshData(); });
};
...
function pushToGitHub() {
    setGhStatus('Uploading...', true);
    fetch('/push_to_github', { method: 'POST' }).then(r => r.json()).then(res => {
        setGhStatus(res.success ? '✅ Uploaded' : '❌ Failed: ' + (res.error || ''), res.success);
    });
}

function pullFromGitHub() {
    setGhStatus('Pulling...', true);
    fetch('/pull_from_github', { method: 'POST' }).then(r => r.json()).then(res => {
        setGhStatus(res.success ? '✅ Pulled' : '❌ Failed', res.success);
...
def dashboard():
    ...
    cfg = load_config()
    return render_template_string(HTML_TEMPLATE, profile_name=cfg["profile_name"], original_gravity=cfg["original_gravity"], today=today, now=now)
        ...
        if os.path.exists(csv_file):
            with open(csv_file, "r") as f:
                ...
                if rows:
                    ...
                    return jsonify({"data": fallback, "timestamp": last.get("timestamp", "Unknown")})
...
# ============================================================
# FIXED WEBHOOK (This should resolve the 400 Bad Request)
# ============================================================
@app.route("/webhook", methods=["POST"])
def webhook():
    global latest_data, last_received_time
    try:
        # Robust parsing for RAPT Pill (handles missing Content-Type)
        if request.is_json:
        ...
        raw_sg = float(data.get("gravity") or 0.0)
        temp_c = float(data.get("temperature") or data.get("temp") or 0.0)

        append_log_entry(now_uk, raw_sg, temp_c)

        threading.Thread(target=push_csv_to_github_background, daemon=True).start()

        print(f"✅ Webhook OK - Temp: {temp_c}°C | Gravity: {raw_sg}")
        return jsonify({"success": True}), 200

    except Exception as e:
        print("❌ Webhook error:", str(e))
        print("Raw payload received:", request.get_data())
        return jsonify({"success": False, "error": str(e)}), 400

# ============================================================
# TEMPERATURE OFFSET ROUTE
# ============================================================
...
# ============================================================
# Keep ALL your other original functions below unchanged:
# start_brew, append_log_entry, download_csv, download_log,
# GitHub helpers (_github_headers, _github_get_file, etc.),
# push_to_github, pull_from_github, push_csv_to_github_background,
# restore_csv_from_github_on_startup, view_log route, etc.
# ============================================================

# Paste the rest of your original code here (everything after the webhook in your previous version)
