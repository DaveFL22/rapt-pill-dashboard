...
# ============================================================
#  BLOCK 2A — DASHBOARD HTML (NO GITHUB BUTTONS)
# ============================================================
...
<body class="bg-zinc-950 text-white p-6">
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-4xl font-bold mb-2">Brew Dashboard</h1>
            <p class="text-zinc-400">Monitor your fermentation in real time.</p>
        </div>

        <!-- Button Row (GitHub buttons removed) -->
        <div class="flex flex-wrap gap-4 mb-6">

            <!-- Start New Brew -->
            <button onclick="openNewBrewModal()"
        ...
        <!-- Status Bar -->
        <div id="statusBar"
             class="bg-zinc-800 text-zinc-300 px-4 py-3 rounded-lg mb-6">
            Loading latest data…
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <div class="bg-zinc-900 p-6 rounded-xl shadow">
                <h2 class="text-lg text-zinc-400 mb-1">Temperature</h2>
                <p id="tempValue" class="text-3xl font-bold">--</p>
            </div>

            <div class="bg-zinc-900 p-6 rounded-xl shadow">
                <h2 class="text-lg text-zinc-400 mb-1">Gravity</h2>
                <p id="gravityValue" class="text-3xl font-bold">--</p>
            </div>

            <div class="bg-zinc-900 p-6 rounded-xl shadow">
                <h2 class="text-lg text-zinc-400 mb-1">Time Running</h2>
                <p id="timeRunning" class="text-3xl font-bold">--</p>
            </div>
        ...
        <div id="newBrewModal"
            ...
            <div class="bg-zinc-900 p-6 rounded-xl w-96 shadow-xl">

                <h2 class="text-xl font-bold mb-4">Start New Brew</h2>

                <label class="block mb-2 text-zinc-300">Brew Name</label>
                <input id="brewNameInput"
                       class="w-full px-3 py-2 rounded bg-zinc-800 text-white mb-4">

                <div class="flex justify-end gap-3">
                    <button onclick="closeNewBrewModal()"
                        class="px-4 py-2 rounded bg-zinc-700 hover:bg-zinc-600">
                        Cancel
                    </button>

                    <button onclick="startNewBrew()"
...
// MODAL HANDLING
// ===============================
function openNewBrewModal() {
    document.getElementById("newBrewModal").classList.remove("hidden");
}

function closeNewBrewModal() {
    document.getElementById("newBrewModal").classList.add("hidden");
}
...
function startNewBrew() {
    const name = document.getElementById("brewNameInput").value.trim();

    if (!name) {
        alert("Please enter a brew name.");
        return;
    }

    fetch('/start_new_brew', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brew_name: name })
    })
    .then(r => r.json())
    .then(res => {
        if (res.success) {
            alert("New brew started.");
            closeNewBrewModal();
        } else {
...
function refreshData() {
    fetch('/latest_data')
        ...
        .then(data => {
            ...
            document.getElementById("gravityValue").textContent = data.gravity_corrected ?? "--";
            document.getElementById("timeRunning").textContent = data.time_running ?? "--";
            document.getElementById("statusBar").textContent = "Live data updated.";
        })
        .catch(() => {
