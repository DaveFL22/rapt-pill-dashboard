...
def view_log():
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            try:
                # Support old logs that used "temperature" and new logs with "temperature_raw"
...
<script>
    ...
    const gravityChart = new Chart(document.getElementById('gravityChart').getContext('2d'), {{
        ...
        options: {{
            ...
            scales: {{ x: {{ ticks: {{ color: '#9ca3af' }}, grid: {{ color: '#111827' }} }}, y: {{ ticks: {{ color: '#9ca3af' }}, grid: {{ color: '#111827' }} }} }},
            plugins: {{
