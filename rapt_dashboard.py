...
function filterYeasts(query) {
    ...
    const matches = allYeasts.filter(y => y.toLowerCase().includes(q)).slice(0, 50)
    ...
    dropdown.innerHTML = matches.map(y =>
        `<div class="px-4 py-2 cursor-pointer hover:bg-zinc-600 text-white text-sm"
            onclick="selectYeast('${y.replace(/'/g, "\\'")}')">${y}</div>`
    ).join('')
    dropdown.classList.remove('hidden')
}

function selectYeast(name) {
    document.getElementById('yeastSearch').value = name
    document.getElementById('yeastNameHidden').value = name
    document.getElementById('yeastDropdown').classList.add('hidden')
...
def yeast_list():
    ...
    try:
        ...
        names = sorted(set(row["Name"].strip() for row in reader if row.get("Name", "").strip()))
        return jsonify({"yeasts": names})
    except Exception as e:
