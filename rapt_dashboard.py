        ...
        .then(result => {
            ...
            if (result.diacetyl_rest) {
                ...
                const savedLabel = savedStart
                    ? ` &nbsp;|&nbsp; 📅 Rest starts: <span class="font-bold text-green-400">${new Date(savedStart).toLocaleString('en-GB', {day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit', hour12:false})}</span>`
                    : ''
                diacetylEl.innerHTML = `
...
def get_latest():
        ...
        if os.path.exists(csv_file):
            with open(csv_file, "r") as f:
                ...
                if rows:
                    ...
                    return jsonify({"data": fallback, "timestamp": last["timestamp"], "session_start": cfg.get("session_start", ""), "profile_name": cfg.get("profile_name", ""), "diacetyl_rest": cfg.get("diacetyl_rest", False), "yeast_name": cfg.get("yeast_name", ""), "fermentation_temp": cfg.get("fermentation_temp"), "diacetyl_start": cfg.get("diacetyl_start", "")})
    ...
    return jsonify({"data": data_to_send, "timestamp": ts, "session_start": cfg.get("session_start", ""), "profile_name": cfg.get("profile_name", ""), "diacetyl_rest": cfg.get("diacetyl_rest", False), "yeast_name": cfg.get("yeast_name", ""), "fermentation_temp": cfg.get("fermentation_temp"), "diacetyl_start": cfg.get("diacetyl_start", "")})
