...
# ============================================================
#  BLOCK 4 — BACKEND ROUTES (UPDATED WITH SECURE PASSWORD CHECK)
# ============================================================
...
# Load secure password from environment (GitHub Secret)
...
def verify_upload_password():
    """
    Securely verifies the password sent from the browser.
    The real password is stored ONLY in the server environment
    (GitHub Secret: UPLOAD_PASSWORD).
    """
...
# ------------------------------------------------------------
#  PUSH CSV TO GITHUB (UNCHANGED)
# ------------------------------------------------------------
@app.route('/push_to_github', methods=['POST'])
def push_to_github():
    """
    Uploads the local CSV log to GitHub.
    This route is only called AFTER password verification.
    """
    try:
        # Load CSV
        with open('fermentation_log.csv', 'r') as f:
            csv_data = f.read()

        # Get latest SHA
        sha_url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{GITHUB_FILE_PATH}"
...
# ------------------------------------------------------------
#  PULL CSV FROM GITHUB (UNCHANGED)
# ------------------------------------------------------------
@app.route('/pull_from_github', methods=['POST'])
def pull_from_github():
    """
    Downloads the CSV from GitHub and overwrites the local file.
    """
    try:
        ...
        with open('fermentation_log.csv', 'w') as f:
            f.write(decoded)
...
def background_restore_csv():
    """
    On startup, restore CSV from GitHub if local file is missing.
    """
    if not os.path.exists("fermentation_log.csv"):
        try:
            url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{GITHUB_FILE_PATH}"
            res = requests.get(url, headers=GITHUB_HEADERS)
...
# ============================================================
#  BLOCK 5B — FERMENTATION LOG VIEWER (UPDATED, SECURE UPLOAD)
# ============================================================
...
<body class="bg-zinc-950 text-white p-6">
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-8">
            ...
            <a href="/" class="bg-zinc-700 hover:bg-zinc-600 px-6 py-3 rounded-2xl text-white font-semibold text-center">
                ← Back to Dashboard
            </a>
        ...
        <!-- TABLE -->
        <div class="overflow-auto bg-zinc-900 p-6 rounded-3xl">
            <table class="w-full text-left text-sm">
                <thead class="text-zinc-400 border-b border-zinc-700">
                    <tr>
                        <th class="py-2">Timestamp</th>
                        <th class="py-2">Temperature (°C)</th>
                        <th class="py-2">Gravity</th>
                        <th class="py-2">Corrected Gravity</th>
                        <th class="py-2">ABV (%)</th>
                        <th class="py-2">Battery (%)</th>
                    </tr>
                </thead>
                <tbody id="logTable" class="text-white"></tbody>
            </table>
        </div>
    ...
    fetch('/get_log')
        ...
        .then(rows => {
            ...
            rows.forEach(row => {
                ...
                tr.innerHTML = `
                    <td class="py-2">${row.timestamp}</td>
                    <td class="py-2">${row.temperature}</td>
                    <td class="py-2">${row.gravity}</td>
                    <td class="py-2">${row.gravity_corrected}</td>
                    <td class="py-2">${row.abv}</td>
                    <td class="py-2">${row.battery}</td>
                `;
...
// ------------------------------------------------------------
//  SECURE GITHUB UPLOAD (PASSWORD VERIFIED SERVER-SIDE)
// ------------------------------------------------------------
function pushToGitHub() {
    ...
    // Stage 3: Ask for password (NOT validated here)
    ...
    .then(res => {
        ...
        if (!res.ok || !res.body.success) {
            setGhStatus("❌ Incorrect password. Upload cancelled.", false);
            return;
        }

        // Password OK → proceed with upload
        ...
        return fetch('/push_to_github', { method: 'POST' })
            ...
            .then(result => {
                if (result.success) {
                    ...
                    setGhStatus("❌ Upload failed: " + (result.error || "Unknown error"), false);
                }
            });
    })
    .catch(err => {
        console.error(err);
        setGhStatus("❌ Error verifying password or uploading.", false);
    });
}
...
function pullFromGitHub() {
    ...
    fetch('/pull_from_github', { method: 'POST' })
        ...
        .catch(err => {
            console.error(err);
            setGhStatus("❌ Network error during pull.", false);
        });
}
