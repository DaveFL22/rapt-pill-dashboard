            ...
            <form id="brewForm">
                ...
                <div id="yeastInfoCard" class="hidden mb-6 p-4 rounded-xl bg-zinc-800 border border-zinc-600 text-sm text-zinc-300 space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-x-4 gap-y-1">
                        <p>🏷️ Brand: <span class="text-white" id="yic_brand"></span></p>
                        <p>🔖 Code: <span class="text-white" id="yic_code"></span></p>
                        ...
                        <p>🍷 Alcohol Tol: <span class="text-white" id="yic_alcohol"></span></p>
                        <p>🧬 Diastaticus: <span class="text-white" id="yic_diastaticus"></span></p>
                        <p>⚡ Kveik: <span class="text-white" id="yic_kveik"></span></p>
                    </div>
                    <div id="yic_equivalent_row" class="hidden">
                        <p>🔄 Equivalent: <span class="text-white" id="yic_equivalent"></span></p>
                    </div>
...
function loadYeasts() {
    if (allYeasts.length > 0) return
    fetch('/yeast_list')
        .then(r => r.json())
        .then(data => {
            allYeasts = data.yeasts || []
        })
        .catch(() => console.log('Could not load yeast list'))
}
...
function selectYeast(idx) {
    ...
    document.getElementById('yic_brand').textContent = y.brand || '--'
    document.getElementById('yic_code').textContent = y.code || '--'
    document.getElementById('yic_type').textContent = y.type || '--'
    document.getElementById('yic_form').textContent = y.form || '--'
    document.getElementById('yic_temp').textContent = (y.temp_low && y.temp_high) ? `${y.temp_low}°C – ${y.temp_high}°C` : '--'
    document.getElementById('yic_attenuation').textContent = (y.attenuation_low && y.attenuation_high) ? `${y.attenuation_low}% – ${y.attenuation_high}%` : '--'
    document.getElementById('yic_flocculation').textContent = y.flocculation || '--'
    document.getElementById('yic_alcohol').textContent = y.alcohol_tolerance ? `${y.alcohol_tolerance}%` : '--'
    document.getElementById('yic_diastaticus').textContent = y.diastaticus || '--'
    document.getElementById('yic_kveik').textContent = y.kveik || '--'

    const eqRow = document.getElementById('yic_equivalent_row')
    if (y.equivalent) {
        document.getElementById('yic_equivalent').textContent = y.equivalent
        ...
        for row in reader:
            ...
            if name and name not in seen:
                ...
                yeasts.append({
                    ...
                    "attenuation_low": row.get("AttenuationLow", "").strip(),
                    "attenuation_high": row.get("AttenuationHigh", "").strip(),
                    "flocculation": row.get("Flocculation", "").strip(),
                    "temp_low": row.get("TempLow", "").strip(),
                    "temp_high": row.get("TempHigh", "").strip(),
                    "alcohol_tolerance": row.get("AlcoholTolerance", "").strip(),
                    "diastaticus": row.get("Diastaticus", "").strip(),
                    "kveik": row.get("Kveik", "").strip(),
                    "equivalent": row.get("Equivalent", "").strip(),
