...
def save_offset(new_offset):
    cfg = load_config()
    cfg["calibration_offset"] = float(new_offset)
...
function refreshData() {
    ...
    fetch('/latest')
        ...
        .then(result => {
            ...
            // Load offset into input
            if (d.calibration_offset !== undefined) {
                document.getElementById('offsetInput').value =
                    parseFloat(d.calibration_offset).toFixed(4)
            }
...
<!-- TEMPERATURE OFFSET BAR -->
<div class="max-w-5xl mx-auto mt-4 bg-zinc-900 p-5 rounded-3xl flex items-center justify-between">
    <div class="text-zinc-400 text-sm font-medium">
        Temperature Offset
    </div>

    <div class="flex items-center gap-3">
        <input id="offsetInput" type="number" step="0.0001"
            class="w-28 p-2 rounded-xl bg-zinc-800 text-white text-sm text-right" />

        <button onclick="saveOffset()"
            class="px-4 py-2 bg-amber-400 hover:bg-amber-300 text-black font-semibold rounded-xl text-sm">
            Save
        </button>
    </div>
</div>
...
def get_latest():
    ...
    data_to_send["calibration_offset"] = cfg["calibration_offset"]
