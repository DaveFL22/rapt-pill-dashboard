                ...
                <label class="block mb-3">
                    ...
                    <div class="flex items-center gap-3 mt-1">
                        <input id="startTime" name="start_time" type="time"
                            class="flex-1 p-3 rounded-xl bg-zinc-800 text-white" />
                        <label class="flex items-center gap-2 cursor-pointer text-zinc-300 text-sm whitespace-nowrap">
                            <input type="checkbox" id="bstCheck" class="w-4 h-4 accent-yellow-400"
                                onchange="applyBST()" />
                            BST (+1hr)
                        </label>
                    </div>
...
function applyBST() {
    const bst = document.getElementById('bstCheck').checked
    const now = new Date()
    if (bst) now.setHours(now.getHours() + 1)
...
function openModal() {
    ...
    document.getElementById('startTime').value = now.toTimeString().slice(0, 5)
    document.getElementById('bstCheck').checked = false
