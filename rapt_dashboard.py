...
def get_current_brew_log_filename():
    """
    Returns the correct log filename for the current brew,
    based on profile_name + session_start date.
    """
    try:
        with open(CONFIG_FILE, "r") as f:
            cfg = json.load(f)
    ...
    # Make profile name filesystem‑safe
...
# ============================================================
#  BASIC FERMENTATION LOGGING (PER‑BREW)
# ============================================================
def ensure_log_file():
    """
    Ensures the per‑brew log file exists.
    Creates fermentation_logs/ folder if needed.
    """
...
def append_log_entry(timestamp, raw_sg, temp_c):
    """
    Appends a new log entry to the current brew's log file.
    """
    filename = get_current_brew_log_filename()
    ensure_log_file()
...
def start_brew():
    ...
    # Next log entry will automatically go into the new per‑brew file
...
def download_csv():
    ...
    csv_name = os.path.basename(filename).replace(".json", ".csv")
