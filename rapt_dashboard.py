...
<body class="bg-zinc-950 text-white min-h-screen p-6">
    ...
    <div id="dryHopGroup1" class="hidden max-w-5xl mx-auto mb-4 rounded-3xl border border-green-700 overflow-hidden">
        <div id="dryHopHeader1" class="p-5 bg-zinc-900 text-lg font-medium text-zinc-400">
            🌿 Dry Hop — First Addition
        </div>
    ...
    <div id="dryHopGroup2" class="hidden max-w-5xl mx-auto mb-4 rounded-3xl border border-green-700 overflow-hidden">
        <div id="dryHopHeader2" class="p-5 bg-zinc-900 text-lg font-medium text-zinc-400">
            🌿 Dry Hop Additions
        </div>
...
function updateDryHopDisplay(result) {
    const group1 = document.getElementById('dryHopGroup1')
    const group2 = document.getElementById('dryHopGroup2')
    ...
    if (!result.dry_hop || !result.dry_hop_count) return
    ...
    if (count === 2) {
        ...
        // Second hop goes AFTER diacetyl rest
        ...
        // ONE HOP: Goes entirely AFTER diacetyl rest
        if (stage === 'drop_to_14' || stage === 'hopping' || stage === 'complete') {
            group2.classList.remove('hidden')
...
function updateDryHopDisplay(result) {
    const group = document.getElementById('dryHopGroup')
    const header = document.getElementById('dryHopHeader')
    const rows = document.getElementById('dryHopRows')
    ...
    // Helper: elapsed time string from a date
    ...
    // Helper: countdown string to a future time
    ...
    function addRow(html, borderTop = true) {
        const div = document.createElement('div')
        div.className = `p-5 bg-zinc-900 text-base font-medium ${borderTop ? 'border-t border-zinc-700' : ''}`
        div.innerHTML = html
        rows.appendChild(div)
    ...
    if (count === 2 && stage === '') {
        // Two hops: first hop goes in at start of diacetyl rest — prompt immediately
        header.innerHTML = `🌿 Dry Hop — 2 Additions`
        addRow(`<span class="text-yellow-400 font-bold">⚠️ Add first dry hop now</span> — this goes in at the start of the Diacetyl Rest
            <button onclick="setDryHopStage('hop1_added')" class="ml-4 px-3 py-1 rounded-xl bg-green-700 hover:bg-green-600 text-white text-sm font-semibold">✅ First Hop Added</button>`, false)

    } else if (count === 2 && stage === 'hop1_added') {
        header.innerHTML = `🌿 Dry Hop — 2 Additions`
        addRow(`✅ First dry hop added at ${fmt(stageTime)} — <span class="text-green-400">in for ${elapsed(stageTime)}</span>`, false)
        addRow(`⏳ Waiting for Diacetyl Rest to complete — then drop to <span class="font-bold text-blue-300">14°C</span> for 12 hours before second hop`)

    } else if (stage === 'drop_to_14') {
        // Drop to 14°C stage — 12 hour countdown
        const due = new Date(stageTime.getTime() + 12 * 60 * 60 * 1000)
        const remaining = due - now
        const over = remaining <= 0
        header.innerHTML = `🌿 Dry Hop — ${count === 2 ? '2 Additions' : '1 Addition'}`
        if (count === 2) addRow(`✅ First dry hop added and fermenting`, false)
        if (over) {
            addRow(`<span class="text-yellow-400 font-bold">⚠️ 14°C hold complete!</span> — <span class="font-bold">Add ${count === 2 ? 'second' : ''} dry hop now</span>
                <button onclick="setDryHopStage('hopping')" class="ml-4 px-3 py-1 rounded-xl bg-green-700 hover:bg-green-600 text-white text-sm font-semibold">✅ Hop${count === 2 ? 's' : ''} Added</button>`, count === 2)
        } else {
            const h = Math.floor(remaining / (1000 * 60 * 60))
            const m = Math.floor((remaining % (1000 * 60 * 60)) / (1000 * 60))
            addRow(`🌡️ Holding at <span class="font-bold text-blue-300">14°C</span> — add ${count === 2 ? 'second' : ''} dry hop in <span class="font-bold text-blue-300">${h}h ${m}m</span>`, count === 2)
        }

    } else if (stage === 'hopping') {
        // Dry hop in fermenter — 3 day max countdown
        const due = new Date(stageTime.getTime() + 3 * 24 * 60 * 60 * 1000)
        const remaining = due - now
        const over = remaining <= 0
        header.innerHTML = `🌿 Dry Hop — ${count === 2 ? '2 Additions' : '1 Addition'}`
        if (count === 2) addRow(`✅ Both dry hops in fermenter`, false)
        else addRow(`✅ Dry hop added at ${fmt(stageTime)}`, false)

        if (over) {
            addRow(`<span class="text-red-400 font-bold animate-pulse">🚨 3 day dry hop period complete — Start Cold Crash!</span>
                <button onclick="startColdCrashNow()" class="ml-4 px-3 py-1 rounded-xl bg-blue-700 hover:bg-blue-600 text-white text-sm font-semibold">❄️ Start Cold Crash</button>`)
        } else {
            const h = Math.floor(remaining / (1000 * 60 * 60))
            const d = Math.floor(h / 24)
            const hr = h % 24
            const m = Math.floor((remaining % (1000 * 60 * 60)) / (1000 * 60))
            addRow(`⏱️ Dry hop time remaining: <span class="font-bold text-green-400">${d}d ${hr}h ${m}m</span> — cold crash after 3 days maximum`)
        }

    } else if (stage === 'complete') {
        header.innerHTML = `🌿 Dry Hop — Complete`
        addRow(`✅ Dry hopping complete — cold crash started`, false)
    }
}
