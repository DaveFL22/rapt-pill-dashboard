# ========================================================
# RAPT Pill Fermentation Dashboard - FULL UPDATED SCRIPT
# ========================================================
...
# ============================================================
# DASHBOARD HTML TEMPLATE (Updated)
# ============================================================
...
function refreshData() {
    ...
    fetch('/latest')
        ...
        .then(result => {
            ...
            if (d.gravity_corrected !== undefined) {
                gravEl.textContent = parseFloat(d.gravity_corrected).toFixed(4);
            } else {
                gravEl.textContent = '1.----';
            }
...
def get_latest():
    ...
    ts = (last_received_time.astimezone(uk).strftime("%H:%M:%S • %d %b") 
          if last_received_time else "Never")

    return jsonify({"data": data_to_send, "timestamp": ts})

# ============================================================
# WEBHOOK (Improved with validation)
# ============================================================
...
def webhook():
    ...
    try:
        ...
        logging.info(f"Webhook received - SG: {raw_sg:.4f}, Temp: {temp_c:.1f}°C")
...
def start_brew():
    try:
        ...
        offset = float(request.form.get("calibration_offset", 0.0))
        ...
        logging.info(f"New brew started: {profile_name} (OG: {og})")
...
# ============================================================
# GITHUB HELPERS & ROUTES (unchanged but kept for completeness)
# ============================================================
def _github_headers():
...
# Auto push, manual push, manual pull, startup restore functions remain the same as your original
# (I've kept them unchanged for brevity - they were already good)

@app.route("/push_to_github", methods=["POST"])
def push_to_github():
    # ... (same as your original Block 4)
    ...
    try:
        ...
        payload = {
            "message": f"Manual upload {filename}",
            "content": content_b64,
...
def pull_from_github():
    ...
    try:
        ...
        content_bytes = base64.b64decode(existing["content"])
...
def push_csv_to_github_background():
    # Same as your original safe version
    try:
        headers = _github_headers()
...
def restore_csv_from_github_on_startup():
    ...
    try:
        existing = _github_get_file(headers, base_url)
        if not existing or not existing.get("content"): return
        content_bytes = base64.b64decode(existing["content"])
        if len(content_bytes) < 10: return
        with open(csv_file, "wb") as f:
            f.write(content_bytes)
        logging.info("Startup: CSV restored from GitHub")
    except Exception as e:
...
def keepalive():
    ...
    while True:
        try:
            requests.get(RENDER_HEALTH_URL, timeout=3)
        except:
            pass
        time.sleep(300)
...
<body class="bg-zinc-950 text-white p-6">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex gap-3 mb-6">
            ...
            <button onclick="applyDateRange()" class="bg-amber-500 hover:bg-amber-400 text-black px-4 py-2 rounded-lg">Apply</button>
        </div>
...
function updateCharts(data) {
    ...
    for (let i = 1; i < data.length; i++) {
        if (data[i-1].gravity_corrected - data[i].gravity_corrected > 0.004) {
            gravityDrops.push({index: i, value: data[i].gravity_corrected});
        }
        if (data[i-1].temperature - data[i].temperature > 2) {
...
// Push / Pull functions (same as before)
function pushCSV() {
    if (!confirm("⚠ WARNING: This will overwrite the file on GitHub. Continue?")) return;
    if (!confirm("Are you ABSOLUTELY sure?")) return;
    fetch('/push_to_github', {method: 'POST'})
        .then(r => r.json())
        .then(res => alert(res.success ? "✅ Uploaded successfully" : "❌ Upload failed: " + (res.error || "")));
}

function pullCSV() {
    if (confirm("Pull latest CSV from GitHub?")) {
        fetch('/pull_from_github', {method: 'POST'})
            .then(r => r.json())
            .then(res => {
                if (res.success) { alert("✅ Pulled successfully"); location.reload(); }
