        ...
        <div class="bg-zinc-900 p-8 rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto my-4">
            ...
            <form id="brewForm">
                ...
                <div id="dryHopFields" class="hidden mb-6 pl-4 border-l-2 border-green-600 space-y-3">
                    <label class="block">
                        <span class="text-zinc-300">Number of dry hop additions</span>
                        <input id="dryHopCount" name="dry_hop_count" type="number" min="1" max="6" value="1"
                            class="w-full mt-1 p-3 rounded-xl bg-zinc-800 text-white"
                            oninput="renderDryHopDayFields(this.value)" />
                    </label>
                    <div id="dryHopDayFields" class="space-y-2"></div>
                </div>
...
function loadCurrentBrew() {
    fetch('/latest')
        ...
        .then(result => {
            ...
            if (dryHopChecked && result.dry_hop_days && result.dry_hop_days.length > 0) {
                document.getElementById('dryHopCount').value = result.dry_hop_days.length
                renderDryHopDayFields(result.dry_hop_days.length)
                result.dry_hop_days.forEach((days, i) => {
                    const input = document.querySelector(`[name="dry_hop_day_${i + 1}"]`)
...
function clearBrewForm() {
    ...
    document.getElementById('dryHopCount').value = 1
    document.getElementById('dryHopDayFields').innerHTML = ''
...
function toggleDryHopFields(checked) {
    const fields = document.getElementById('dryHopFields')
    fields.classList.toggle('hidden', !checked)
    if (checked) renderDryHopDayFields(document.getElementById('dryHopCount').value)
}

function renderDryHopDayFields(count) {
    const container = document.getElementById('dryHopDayFields')
    const n = Math.max(1, Math.min(6, parseInt(count) || 1))
    const existing = container.querySelectorAll('input[type="number"]')
    const existingVals = Array.from(existing).map(i => i.value)
    container.innerHTML = ''
    for (let i = 0; i < n; i++) {
        const val = existingVals[i] || ''
        container.innerHTML += `
        <label class="block">
            <span class="text-zinc-400 text-sm">Addition ${i + 1} — after how many days from brew start?</span>
            <input name="dry_hop_day_${i + 1}" type="number" min="1" step="1" value="${val}"
                placeholder="e.g. 4"
                class="w-full mt-1 p-3 rounded-xl bg-zinc-800 text-white text-sm" />
        </label>`
    }
}
...
function stopDiacetylRestNow() {
    ...
    fetch('/set_diacetyl_end', { method: 'POST' })
        ...
        .then(res => {
            if (res.success) {
                // If cold crash is configured, start it automatically
                fetch('/latest').then(r => r.json()).then(result => {
                    if (result.cold_crash_type && result.cold_crash_type !== 'none') {
                        fetch('/set_cold_crash_start', { method: 'POST' })
                            .then(() => refreshData())
                    } else {
                        refreshData()
                    }
...
function updateDryHopDisplay(result) {
    ...
    if (!result.dry_hop || !result.dry_hop_days || result.dry_hop_days.length === 0) {
        group.classList.add('hidden')
        return
    }

    group.classList.remove('hidden')
    const brewStart = new Date(result.session_start)
    const now = new Date()
    rows.innerHTML = ''

    result.dry_hop_days.forEach((days, i) => {
        const dueDate = new Date(brewStart.getTime() + days * 24 * 60 * 60 * 1000)
        const diffMs = dueDate - now
        const diffHours = Math.floor(Math.abs(diffMs) / (1000 * 60 * 60))
        const diffDays = Math.floor(diffHours / 24)
        const diffHrsOnly = diffHours % 24
        const overdue = diffMs < 0
        const due = Math.abs(diffMs) < 60 * 60 * 1000 // within 1 hour

        let colour, status, timeStr
        if (due) {
            ...
            colour = 'text-red-400'
            status = '🔴 Overdue by'
            timeStr = `${diffDays}d ${diffHrsOnly}h`
        } else {
        ...
        const dueDateStr = dueDate.toLocaleString('en-GB', {
            day: '2-digit', month: 'short', year: 'numeric',
            hour: '2-digit', minute: '2-digit', hour12: false
        })

        const borderClass = i > 0 ? 'border-t border-zinc-700' : ''
        rows.innerHTML += `
            <div class="p-5 bg-zinc-900 ${borderClass}">
                <div class="flex justify-between flex-wrap gap-2 text-lg font-medium">
                    <span class="${colour}">🌿 Addition ${i + 1}</span>
                    <span class="text-zinc-300">Day ${days} — ${dueDateStr}</span>
                    <span class="${colour}">${status} ${timeStr}</span>
                </div>
            </div>`
    })
}
...
def start_brew():
    ...
    dry_hop_count = int(request.form.get("dry_hop_count", 0)) if dry_hop else 0
    dry_hop_days = []
    for i in range(1, dry_hop_count + 1):
        val = request.form.get(f"dry_hop_day_{i}", "").strip()
        if val:
            dry_hop_days.append(int(val))
