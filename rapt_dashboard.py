...
# ============================================================
# Block 5A — LOG VIEWER HTML + JAVASCRIPT (CLEAN + FIXED)
# ============================================================
...
// ===============================
function showModal(messageHTML, onConfirm) {
    const overlay = document.createElement("div");
    overlay.className = "fixed inset-0 bg-black/70 flex items-center justify-center z-50";

    const box = document.createElement("div");
    box.className = "bg-zinc-900 p-8 rounded-2xl max-w-md w-full text-center shadow-xl";

    box.innerHTML = `
        <div class="mb-6 text-lg leading-relaxed">${messageHTML}</div>
        <div class="flex justify-center gap-4">
            <button id="modalCancel"
                class="px-5 py-2 rounded-xl bg-zinc-700 hover:bg-zinc-600">
                Cancel
            </button>
            <button id="modalOK"
                class="px-5 py-2 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-semibold">
                OK
            </button>
        </div>
    `;
...
// ===============================
// UPDATED pushCSV() WITH CENTERED MODALS
// ===============================
function pushCSV() {

    // FIRST WARNING — centered modal
    showModal(
        `
        <strong class="text-yellow-300 text-xl">⚠ WARNING ⚠</strong><br><br>
        You are about to <strong>REPLACE</strong> the CSV file on GitHub.<br>
        If the GitHub file has more data than this log file,<br>
        <strong>YOU MAY LOSE DATA.</strong><br><br>
        Recommended: Take a backup of the CSV on GitHub first.
        `,
        () => {

            // SECOND WARNING — centered, RED, CAPITALIZED
            showModal(
                `
