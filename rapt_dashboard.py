...
# ============================================================
# ROBUST LOG VIEWER HTML (Fixed buttons + graphs)
# ============================================================
LOG_VIEWER_HTML = """
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-zinc-950 text-white p-6">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex flex-wrap gap-3 mb-6">
            <button id="btn1"  class="filter-btn px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm">Last 1h</button>
            <button id="btn6"  class="filter-btn px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm">Last 6h</button>
            <button id="btn12" class="filter-btn px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm">Last 12h</button>
            <button id="btn24" class="filter-btn px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm">Last 24h</button>
            <button id="btn48" class="filter-btn px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm">Last 48h</button>
            <button id="btnAll" class="filter-btn px-5 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm">Show All</button>
        </div>

        <div class="flex gap-3 mb-10 items-end">
            <div>
                <label class="text-xs text-zinc-400 block mb-1">Start</label>
                <input type="datetime-local" id="startDateTime" class="bg-zinc-800 text-white px-3 py-2 rounded-lg">
            </div>
            <div>
                <label class="text-xs text-zinc-400 block mb-1">End</label>
                <input type="datetime-local" id="endDateTime" class="bg-zinc-800 text-white px-3 py-2 rounded-lg">
            </div>
            <button onclick="applyDateRange()" 
                    class="bg-amber-500 hover:bg-amber-400 text-black px-5 py-2 rounded-lg font-medium">
                Apply Range
            </button>
        </div>

        <div class="flex gap-3 mb-10">
            <button onclick="pullCSV()" class="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-amber-300 px-4 py-2 rounded-lg">
                ⬇ Pull from GitHub
            </button>
            <button onclick="pushCSV()" class="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-emerald-400 px-4 py-2 rounded-lg">
                ⬆ Upload to GitHub
            </button>
        </div>

        <div id="chartContainer">
            <h2 class="text-xl font-semibold mb-3">Gravity Trend</h2>
            <div class="bg-zinc-900 p-4 rounded-2xl mb-10">
                <canvas id="gravityChart" height="120"></canvas>
            </div>

            <h2 class="text-xl font-semibold mb-3">Temperature Trend (°C)</h2>
            <div class="bg-zinc-900 p-4 rounded-2xl">
                <canvas id="tempChart" height="120"></canvas>
            </div>
        </div>

        <div id="noDataMessage" class="hidden text-center py-20 text-zinc-400">
            No data available yet.<br>
            Please wait for the RAPT Pill to send data or pull from GitHub.
        </div>
    </div>
    ...
    gravityChart = new Chart(document.getElementById("gravityChart"), {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: data.map(e => e.gravity_corrected),
                borderColor: "#22c55e",
                tension: 0.2,
                borderWidth: 3,
                pointRadius: 0
...
function applyFilter(hours) {
    ...
    const filtered = logData.filter(entry => {
        try {
            return !cutoff || new Date(entry.timestamp) >= cutoff;
        } catch(e) {
            return false;
        }
    });
...
function pushCSV() {
    if (confirm("⚠ This will overwrite the file on GitHub.\\n\\nContinue?") && confirm("Are you sure?")) {
        fetch('/push_to_github', {method: 'POST'})
            .then(r => r.json())
            .then(res => alert(res.success ? "✅ Uploaded successfully" : "❌ Upload failed"))
            .catch(() => alert("Network error"));
    }
}

function pullCSV() {
    if (confirm("Download latest log from GitHub?")) {
        fetch('/pull_from_github', {method: 'POST'})
            .then(r => r.json())
            .then(res => {
                if (res.success) {
                    alert("✅ Pulled successfully - refreshing...");
                    location.reload();
                } else {
...
window.onload = function() {
    console.log("Log Viewer loaded with", logData.length, "entries");
...
document.getElementById("btn1").onclick  = () => applyFilter(1);
document.getElementById("btn6").onclick  = () => applyFilter(6);
document.getElementById("btn12").onclick = () => applyFilter(12);
document.getElementById("btn24").onclick = () => applyFilter(24);
document.getElementById("btn48").onclick = () => applyFilter(48);
